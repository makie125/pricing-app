...
export default function App() {
  ...
  const [optionalAddons, setOptionalAddons] = useState(() => {
    const saved = loadSaved('optionalAddons', []);
  ...
  const loadTemplate = (tmpl) => {
    ...
    if (d.optionalAddons !== undefined) setOptionalAddons(Array.isArray(d.optionalAddons) ? d.optionalAddons : []);
        ...
        {activeTab === 'templates' ? (
          ...
          <div className={`backdrop-blur-xl border rounded-3xl p-8 ${t.card}`}>
            ...
            <div className="space-y-4">
              {terms.map(term => (
                <div key={term.id} className={`p-4 border rounded-xl transition-all ${term.enabled ? t.card : 'opacity-50 ' + t.card}`}>
                  <div className="flex items-start gap-3">
                    ...
                    {term.isCustom && (
                      <button onClick={() => setTerms(prev => prev.filter(t => t.id !== term.id))}
                        className="text-red-400/60 hover:text-red-400 transition-colors flex-shrink-0" title="Delete custom term">
              ...
              <button onClick={() => setTerms(prev => [...prev, { id: `custom-term-${Date.now()}`, enabled: true, text: 'Enter your custom term here...', editing: true, isCustom: true }])}
                className="w-full p-4 border-2 border-dashed rounded-xl text-sm text-orange-400 hover:text-orange-300 hover:border-orange-400/50 font-medium flex items-center justify-center gap-2 transition-all">
